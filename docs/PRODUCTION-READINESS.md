# Oracle-Native-Standby Standby Manager v26 — Production Readiness
...
- The included `conf/oracle-native-standby.drv` is intentionally blank for environment-specific required fields, preventing accidental execution against example hosts.
...
- A dedicated `LISTENER_ONS` auxiliary listener is used rather than overwriting the service-managed default listener definition.
- TNS changes use a bounded ONS-managed block and preserve a backup of the prior tnsnames.ora.
