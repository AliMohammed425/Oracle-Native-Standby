...
# Supported oracle-native-standby.drv keys.  Keep this list explicit so the driver file
# cannot execute arbitrary shell code.
ONS_DRV_ALLOWED_KEYS="
SOURCE_DB_NAME
...
TDE_ENCRYPTION_MODE
TDE_ENCRYPT_ALGORITHM
TDE_ENCRYPT_CONTAINER
DG_ARCHIVE_DEST_ID
FULL_BUILD_BACKGROUND

TOOLKIT_OS_USER
ORACLE_OS_USER
REQUIRE_LOCAL_SUDO_ORACLE
REQUIRE_TARGET_ORACLE_SSH
REQUIRE_SOURCE_ORACLE_SSH
SSH_CONNECT_TIMEOUT
"

drv_key_allowed(){
  local key="$1"
  grep -qx "$key" <<<"$ONS_DRV_ALLOWED_KEYS"
}
...
print_driver_file_summary(){
  ...
  echo "TDE encryption : ${TDE_ENCRYPTION_MODE:-NONE} (${TDE_ENCRYPT_ALGORITHM:-AES256})"
}

create_driver_template(){
  local out="${1:-$ONS_ROOT/conf/oracle-native-standby.drv}"
  umask 077
  cat > "$out" <<'EOF'
# ==============================================================================
# Oracle-Native-Standby Physical Standby Driver File
# File name: oracle-native-standby.drv
#
# Format: KEY=VALUE
...
TARGET_AUX_LISTENER_NAME=LISTENER_ONS
...
# OCI PLATFORM
# AUTO | DBAASCLI_PLATFORM | BASE_DB_SERVICE_DBCLI | DBCA_PLATFORM
...
write_driver_file(){
  ...
  umask 077
  cat > "$out" <<EOF2
# Generated by Oracle-Native-Standby. Secrets intentionally omitted.
...
TARGET_AUX_LISTENER_NAME=${TARGET_AUX_LISTENER_NAME:-LISTENER_ONS}
