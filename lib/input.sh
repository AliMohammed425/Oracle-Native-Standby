...
collect_full_build_inputs(){
  echo
  echo "======================================================================"
  echo " Oracle-Native-Standby - OCI Standby Complete Build Wizard"
  echo "======================================================================"
  ...
  prompt_value TARGET_AUX_LISTENER_NAME "Dedicated RMAN auxiliary listener name" "LISTENER_ONS"
...
write_runtime_config(){
  local out="${1:-$ONS_ROOT/conf/standby.env}"
  umask 077
  mkdir -p "$(dirname "$out")"
  cat > "$out" <<EOF
# Generated by Oracle-Native-Standby Complete Build Wizard
