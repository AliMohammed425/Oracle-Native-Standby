...
task_submit_background(){
  ...
  setsid "$ONS_ROOT/bin/oracle-native-standby" internal-selected "$jid" "$kind" "$selector" >>"$out" 2>&1 < /dev/null &
  local pid=$! pgid
  ...
  echo "Status   : $ONS_ROOT/bin/oracle-native-standby status $jid"
  echo "Logs     : $ONS_ROOT/bin/oracle-native-standby logs $jid"
}
...
task_failed_from_background(){
  ...
  setsid "$ONS_ROOT/bin/oracle-native-standby" internal-failed-from "$jid" >>"$out" 2>&1 < /dev/null &
  local pid=$! pgid
  pgid="$(ps -o pgid= -p "$pid" 2>/dev/null | tr -d ' ' || true)"
  pgid="${pgid:-$pid}"
  job_meta_set "$jid" pid "$pid"
