...
_submit_detached(){
  ...
  mkdir -p "$(job_dir "$jid")"
  setsid "$ONS_ROOT/bin/oracle-native-standby" "internal-${action}" "$jid" >>"$out" 2>&1 < /dev/null &
  ...
  echo "Submitted background job: $jid"; echo "PID/PGID: $pid/$pgid"; echo "Status: $ONS_ROOT/bin/oracle-native-standby status $jid"; echo "Logs: $ONS_ROOT/bin/oracle-native-standby logs $jid"
}
