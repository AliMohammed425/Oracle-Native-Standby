...
configure_tns_on_host(){
  ...
  ssh "$user@$host" "mkdir -p '$admin'; touch '$file'; cp -p '$file' '$file.ons.bak' 2>/dev/null || true; awk '/# ONS BEGIN/{skip=1} /# ONS END/{skip=0;next} !skip{print}' '$file' > '$file.ons'; cat >> '$file.ons' <<'TNS'
# ONS BEGIN
${SOURCE_TNS_ALIAS} =
...
# ONS END
TNS
mv '$file.ons' '$file'; chmod 600 '$file'"
}
...
aux_listener_configure(){
  ...
  local lname="${TARGET_AUX_LISTENER_NAME:-LISTENER_ONS}"
  local admin="${TARGET_ORACLE_HOME}/network/admin"
  local file="${admin}/listener.ora"
  ssh "$TARGET_OS_USER@$TARGET_HOST" "mkdir -p '$admin'; touch '$file'; cp -p '$file' '$file.ons.bak' 2>/dev/null || true; awk '/# ONS AUX BEGIN/{skip=1} /# ONS AUX END/{skip=0;next} !skip{print}' '$file' > '$file.ons'; cat >> '$file.ons' <<'LST'
# ONS AUX BEGIN
${lname} =
...
# ONS AUX END
LST
mv '$file.ons' '$file'; chmod 600 '$file'; export ORACLE_HOME='$TARGET_ORACLE_HOME'; export PATH=\$ORACLE_HOME/bin:\$PATH; lsnrctl stop '$lname' >/dev/null 2>&1 || true; lsnrctl start '$lname'; lsnrctl status '$lname'"
}
