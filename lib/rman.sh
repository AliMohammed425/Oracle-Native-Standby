...
rman_active_duplicate(){
  ...
  local tmp; tmp="$(mktemp /tmp/ons_active_dup.XXXXXX)"; chmod 600 "$tmp"; trap 'rm -f "$tmp"' RETURN
  local cfg
...
RMAN
  scp -q "$tmp" "$TARGET_OS_USER@$TARGET_HOST:/tmp/ons_active_duplicate.rman"
  ssh "$TARGET_OS_USER@$TARGET_HOST" "set -e; chmod 600 /tmp/ons_active_duplicate.rman; export ORACLE_HOME='$TARGET_ORACLE_HOME'; export ORACLE_SID='$TARGET_SID'; export PATH=\$ORACLE_HOME/bin:\$PATH; rman cmdfile=/tmp/ons_active_duplicate.rman log=/tmp/ons_active_duplicate.log; rc=\$?; rm -f /tmp/ons_active_duplicate.rman; exit \$rc"
}
...
rman_stage_backup(){
  ...
  case "${method^^}" in
    ...
    RSYNC)
      ...
      ;;
    SCP)
      local td; td="$(mktemp -d /tmp/ons_backup.XXXXXX)"; trap 'rm -rf "$td"' RETURN
...
SQL
  ...
  [[ "$cfg" == DG_CONFIG=* ]] || cfg="DG_CONFIG=(${SOURCE_DB_UNIQUE_NAME},${TARGET_DB_UNIQUE_NAME})"
  local tmp; tmp="$(mktemp /tmp/ons_offline_dup.XXXXXX)"; chmod 600 "$tmp"; trap 'rm -f "$tmp"' RETURN
  cat > "$tmp" <<RMAN
CONNECT AUXILIARY "sys/${TARGET_SYS_PASSWORD}@${TARGET_TNS_ALIAS} AS SYSDBA";
DUPLICATE DATABASE FOR STANDBY
  ...
  NOFILENAMECHECK;
RMAN
  scp -q "$tmp" "$TARGET_OS_USER@$TARGET_HOST:/tmp/ons_offline_duplicate.rman"
  ssh "$TARGET_OS_USER@$TARGET_HOST" "set -e; chmod 600 /tmp/ons_offline_duplicate.rman; export ORACLE_HOME='$TARGET_ORACLE_HOME'; export ORACLE_SID='$TARGET_SID'; export PATH=\$ORACLE_HOME/bin:\$PATH; rman cmdfile=/tmp/ons_offline_duplicate.rman log=/tmp/ons_offline_duplicate.log; rc=\$?; rm -f /tmp/ons_offline_duplicate.rman; exit \$rc"
}
